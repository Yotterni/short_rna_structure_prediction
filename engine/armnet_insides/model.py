    ...
    def forward(self, x, adj, mask, bpp_mask):
        ...
        for ind, mod in enumerate(self.layers):
            if ind < len(self.conv_layers):
                ...
                # raw_attn = raw_attn.masked_fill(~bpp_mask.unsqueeze(1), 0) #TODO: restore it!!!
...
class ShapeArmNet(nn.Module):
    def __init__(self,  
                 adj_ks: int = 3,
                 num_convs: Optional[int] = 3, # None
                 dim=192, 
                 depth=12,
                 head_size=32,
                 attn_kernel_size: int = 17,
...
class StrucArmNet(nn.Module):
    def __init__(self,  
        ...
        self.initial_projection = nn.Linear(input_dim, dim) if input_dim != dim else nn.Identity()
        ...
        self.final_projection = nn.Linear(dim, 3)
    # технофеодализм
            
    def forward(self, embedded_seq, x0):
        """
        ...
        x = self.initial_projection(embedded_seq)
