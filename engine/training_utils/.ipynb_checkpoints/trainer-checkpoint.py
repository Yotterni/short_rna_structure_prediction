...
def train_step(model: nn.Module,
    ...
    for batch in loader.epoch(bin_num):
...
class MaskedLoss(nn.Module):
    ...
    def forward(self, x_predicted: torch.Tensor,
        ...
        """
        return ((x_predicted - x_true) ** 2 * loss_mask).sum() / (1 - loss_mask).sum()
...
def run_experiment(model: nn.Module,
    ...
    for bin_num in range(0, rf_train.num_bins):
        print(f'Bin num: {bin_num}')
        for epoch in range(num_epochs), leave=True:
            print(f'Epoch: {epoch}')
...
class ArmnetLikeModel(armnet_insides.model.ArmNet):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.final_projection = nn.Linear(kwargs['dim'], 5)
...
model = ArmnetLikeModel(depth=8, dim=256).to(struc_device) #dim=192
number_of_epochs = 3 # 5
