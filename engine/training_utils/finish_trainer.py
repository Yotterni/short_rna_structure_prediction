...
rf_train = BinnedDataset('data/rf_dataset/train', 10)
rf_val = BinnedDataset('data/rf_dataset/val', 10)
rf_test = BinnedDataset('data/rf_dataset/test', 10)
...
class MaskedLoss(nn.Module):
    ...
    def forward(self, x_predicted: torch.Tensor,
        ...
        """
        # return ((x_predicted - x_true) ** 2 * loss_mask).sum() / (1 - loss_mask).sum() MSE was used to train bins [0, 2]
        return ((x_predicted - x_true).abs() * loss_mask).sum() / (1 - loss_mask).sum()
...
def run_experiment(model: nn.Module,
    ...
    for bin_num in range(3, rf_train.num_bins):
...
# model = ArmnetLikeModel(depth=8, dim=256)
with open('results_for_bin_2.pkl', 'rb') as file:
    model, _, _ = pickle.load(file)
model.to(struc_device)  #dim=192
number_of_epochs = 5 # 5
